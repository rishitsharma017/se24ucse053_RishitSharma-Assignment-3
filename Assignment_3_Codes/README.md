...
1. **Dijkstra's Algorithm** on Indian city road networks
2. **UGV (Unmanned Ground Vehicle) Pathfinding** in a battlefield grid with static obstacles
3. **UGV Pathfinding** in a dynamic obstacle environment with real-time replanning
...
1. UGV plans an initial path using A\* from start to goal
2. At each step, the environment **updates** — obstacles randomly appear/disappear
3. UGV checks if its remaining planned path is **still obstacle-free**
4. If a **blocked cell** is detected on the path → **replan immediately** from current position
5. UGV continues moving step by step until goal is reached or max steps exceeded
